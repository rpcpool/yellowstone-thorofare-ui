...
const TimeAxis: React.FC<TimeAxisProps> = ({
  ...
  return <div className="relative h-full overflow-hidden w-full">{ticks}</div>;
};
...
export function Timeline({
  ...
  const processStages = (
    ...
    const firstShred =
      transitions.find((t) => t.status === "FirstShredReceived")?.timestamp ||
      0;
    const completed =
      transitions.find((t) => t.status === "Completed")?.timestamp || 0;
    const createdBank =
      transitions.find((t) => t.status === "CreatedBank")?.timestamp || 0;
    const processed =
      transitions.find((t) => t.status === "Processed")?.timestamp || 0;
    const confirmed =
      transitions.find((t) => t.status === "Confirmed")?.timestamp || 0;

    const otherFirstShred =
      otherEndpoint.transitions.find((t) => t.status === "FirstShredReceived")
        ?.timestamp || 0;
    const otherConfirmed =
      otherEndpoint.transitions.find((t) => t.status === "Confirmed")
        ?.timestamp || 0;

    // first shred delay
    if (
      endpoint.first_shred_delay_ms !== null &&
      endpoint.first_shred_delay_ms !== undefined &&
      endpoint.first_shred_delay_ms > 0
    ) {
      stages.push({
        ...
        label: `${
          STAGE_LABELS.first_shred_delay
        } ${endpoint.first_shred_delay_ms.toFixed(1)}ms`,
        parallel: false,
        parallelIndex: 0,
      });
    }
    ...
    if (visibleStages.download && firstShred && completed) {
      stages.push({
        ...
        label: `${STAGE_LABELS.download} ${Math.round(
          endpoint.durations.download_ms
        )}ms`,
        parallel: false,
        parallelIndex: 0,
      });
    ...
    if (visibleStages.replay && createdBank && processed) {
      stages.push({
        ...
        label: `${STAGE_LABELS.replay} ${Math.round(
          endpoint.durations.replay_ms
        )}ms`,
        parallel: false,
        parallelIndex: 0,
      });
    ...
    // confirmation delay - only if this endpoint was slower
    if (
      endpoint.confirmation_delay_ms !== null &&
      endpoint.confirmation_delay_ms !== undefined &&
      endpoint.confirmation_delay_ms > 0 &&
      visibleStages.confirmation
    ) {
      ...
      stages.push({
        ...
        label: `${
          STAGE_LABELS.confirmation_delay || "Confirm Delay"
        } ${endpoint.confirmation_delay_ms.toFixed(1)}ms`,
        parallel: false,
        parallelIndex: 0,
      });
    }
    ...
    if (visibleStages.confirmation && processed && confirmed) {
      stages.push({
        ...
        label: `${STAGE_LABELS.confirmation} ${Math.round(
          endpoint.durations.confirmation_ms
        )}ms`,
        parallel: false,
        parallelIndex: 0,
      });
    ...
    for (let i = 0; i < stages.length; i++) {
      ...
      if (
        stages[i].type === "first_shred_delay" ||
        stages[i].type === "confirmation_delay"
      )
        continue;

      for (let j = 0; j < i; j++) {
        if (
          stages[j].type === "first_shred_delay" ||
          stages[j].type === "confirmation_delay"
        )
          continue;

        if (
          stages[i].startTime < stages[j].endTime &&
          stages[i].endTime > stages[j].startTime
        ) {
    ...
    const processingStages = stages.filter(
      (s) => s.type === "replay" || s.type === "confirmation"
    );

    if (processingStages.length > 0) {
      const maxProcessingIndex = Math.max(
        ...processingStages.map((s) => s.parallelIndex)
      );
  ...
  const getSlotTimeRange = (
    ...
    if (visibleStages.confirmation && confirmed > lastTime)
      lastTime = confirmed;

    return { start: firstShred, end: lastTime };
  };

  const assignSlotLanes = (
  ...
  const firstTimestamp = Math.min(
    ...data.slots.flatMap((slot) => {
      const ep1First =
        slot.endpoint1.transitions.find(
          (t) => t.status === "FirstShredReceived"
        )?.timestamp || Infinity;
      const ep2First =
        slot.endpoint2.transitions.find(
          (t) => t.status === "FirstShredReceived"
        )?.timestamp || Infinity;
  ...
  const renderStages = (
    ...
    return stages.map((stage, idx) => {
      ...
      const y =
        yOffset +
        SLOT_HEIGHT / 2 +
        stage.parallelIndex * (STAGE_HEIGHT + STAGE_SPACING);

      const isDelay =
        stage.type === "first_shred_delay" ||
        stage.type === "confirmation_delay";
      ...
      return (
        <div
          ...
          className={cn(
            ...
            isDelay
              ? `${STAGE_COLORS.waiting} opacity-70 border-2 border-dashed border-gray-300`
              : STAGE_COLORS[stage.type as keyof typeof STAGE_COLORS]
          )}
          style={{
        ...
        >
          {width > 60 && (
            <span className="truncate px-2 text-[11px]">{stage.label}</span>
          )}
          {width > 30 && width <= 60 && (
            <span className="truncate px-1 text-[10px]">
              {isDelay ? "⏱" : stage.type.substring(0, 1).toUpperCase()}{" "}
              {Math.round(stage.duration)}
            </span>
          )}
          {width <= 30 && isDelay && <span className="text-[10px]">⏱</span>}
        </div>
      );
    });
  };
  ...
  return (
    <div className="bg-card rounded-lg border overflow-hidden w-full max-w-full">
      <div
        className="sticky top-0 z-30 bg-card border-b overflow-hidden"
      ...
      >
        ...
        >
          ...
          <div
            ...
            style={{ height: ep1Height }}
          >
            <div className="absolute left-4 top-4 z-10">
              <span
                className={cn("text-sm font-semibold", ENDPOINT_COLORS.ep1)}
              >
                {endpointNames?.[0] ||
                  parseEndpointName(data.endpoints[0].endpoint)}{" "}
                ({maxEp1Lanes} lane{maxEp1Lanes > 1 ? "s" : ""})
              </span>
            </div>
            ...
            {ep1Slots
              .filter(({ slot }) => {
                const slotX =
                  ((slot.endpoint1.transitions[0]?.timestamp ||
                    firstTimestamp) -
                    firstTimestamp) *
                  pixelsPerMs;
                return (
                  slotX > -(viewportOffset || 0) - 200 &&
                  slotX < (viewportOffset || 0) + containerWidth + 200
                );
              })
              .map(({ slot, lane, startTime, endTime }) => (
                ...
                >
                  {/* slot background */}
                  <div
                    className="absolute bg-muted/10 rounded-sm"
                    style={{
                      left: `${
                        (startTime - firstTimestamp) * pixelsPerMs - 2
                      }px`,
                  ...
                  >
                    <span className="absolute -top-6 left-1 text-xs text-muted-foreground bg-card px-1 rounded">
                      {slot.slot}
                    </span>
                  </div>
          ...
          <div
            ...
            style={{ top: ep1Height + 40, height: ep2Height }}
          >
            <div className="absolute left-4 top-4 z-10">
              <span
                className={cn("text-sm font-semibold", ENDPOINT_COLORS.ep2)}
              >
                {endpointNames?.[1] ||
                  parseEndpointName(data.endpoints[1].endpoint)}{" "}
                ({maxEp2Lanes} lane{maxEp2Lanes > 1 ? "s" : ""})
              </span>
            </div>
            ...
            {ep2Slots
              .filter(({ slot }) => {
                const slotX =
                  ((slot.endpoint2.transitions[0]?.timestamp ||
                    firstTimestamp) -
                    firstTimestamp) *
                  pixelsPerMs;
                return (
